...
namespace 未来シェア
{
    public partial class Form1 : Form
    {
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            if ((DateTime.Now - dtLastCheck).TotalSeconds > 10)
            {
                ...
                if (status == DEMAND_STATUS.計算完了)
                {
                    ...
                    if (!mDataOP.UpdateMirai_手動登録(0, ref msg))
                    {
                        messageLog("UpdateMirai_手動登録():失敗" + msg);
                    }
                    messageLog("UpdateMirai_手動登録():終了");
        ...
        private void dataGridView2_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            drawMapAynori(true,mCurrentIndex);
        }
        ...
        void drawMapAynori(bool ToCenter, int index)
        {
            var msg = "";
            ...
            for (int i = 0; i < tbl.Rows.Count; i++)
            {
                ...
                var lat = (double)(tbl.Rows[i]["position_Lat"]);
                var lon = (double)(tbl.Rows[i]["position_Lng"]);
                var eventTime = ((DateTime)(tbl.Rows[i]["イベント日時"])).ToShortTimeString();
                ...
                if (stat == "乗車")
                {
                    ...
                    {
                        mGoogleMaps.CallJS_addInfoWindowLabel2(id, lat1, lng1, 0, 0);
                       // mGeckoWebBrowser.AddMessageEventListener("GoogleMap_MouseClick_"+id, mGoogleMaps.GoogleMap_MouseClick);

                        //mGoogleMaps.CallJS_addInfoWindowLabel2(eventTime, lat1, lng1, 0, 0);
                        var mCssTextForTime = string.Format("opacity: 0.8;font-size: {0}pt ; border: 1px solid #0000ff ; margin-top: 0px ; background: white ; padding: 0px;textAlign: center ; ", 15);
                        var offsetXForTime = -10 - 10;
                        var offsetYForTime = -10 - 10;
        ...
        private void radio_Label_CheckedChanged(object sender, EventArgs e)
        {
            ...
            //
            drawMapAynori(false, mCurrentIndex);
            if (mCssText != "")
            {
